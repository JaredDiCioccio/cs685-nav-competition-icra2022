...
namespace my_local_planner
{
                                    ...
                                    costmap_2d::Costmap2DROS *costmap_ros_)
    {
        if (!initialized_)
        {
            ...
            vis_pub = gn.advertise<visualization_msgs::Marker>("visualization_marker", 0);
    ...
    bool MyLocalPlanner::computeVelocityCommands(geometry_msgs::Twist &cmd_vel)
    {
        ...
        double projectedX = 0.75 * cos(yaw) + curr_pose.pose.position.x;
        double projectedY = 0.75 * sin(yaw) + curr_pose.pose.position.y;

        visualization_msgs::Marker marker;
        marker.header.frame_id = "base_link";
        marker.header.stamp = ros::Time();
        marker.ns = "my_local_planner";
        marker.id = markerId++;
        marker.type = visualization_msgs::Marker::SPHERE;
        marker.action = visualization_msgs::Marker::ADD;
        marker.pose.position.x = projectedX;
        marker.pose.position.y = projectedY;
